...
function createHeroCard(sess: builder.Session, title: string, status: string): builder.HeroCard {
    return new builder.HeroCard(sess)
        ...
        .text(status)
        .images([
            builder.CardImage.create(sess, "https://cdn.october.codes/images/train.jpg")
        ])
        .buttons([
            builder.CardAction.openUrl(sess, "https://amtrak.com", "Track")
        ]);
}
...
bot.dialog("/results", [
    ...
    (sess, result) => {
        if(result.response.entity === "Yes") {
            ...
            let status = `The current status of the train from ${route.from} to ${route.to} is ${route.status}`;
            sess.send(new builder.Message(sess).addAttachment(createHeroCard(sess, route.train, status)));
        }
        else {
            sess.replaceDialog("/");
        }
